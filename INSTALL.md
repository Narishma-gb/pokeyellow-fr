...
These instructions explain how to set up the tools required to build **pokeyellow-fr**, including [**rgbds**](https://github.com/gbdev/rgbds), which assembles the source files into a ROM.
...
WSL has its own file system that's not accessible from Windows, but Windows files *are* accessible from WSL. So you're going to want to install pokeyellow-fr within Windows. You'll have to change the **current working directory** every time you open WSL.

For example, if you want to store pokeyellow-fr in **C:\Users\\*\<user>*\Desktop**, enter this command:
...
Cygwin has its own file system that's within Windows, at **C:\cygwin64\home\\*\<user>***. If you don't want to store pokeyellow-fr there, you'll have to change the **current working directory** every time you open Cygwin.

For example, if you want to store pokeyellow-fr in **C:\Users\\*\<user>*\Desktop**:
...
Now you're ready to [build **pokeyellow-fr**](#build-pokeyellow-fr).
...
To install the software required for **pokeyellow-fr**:
...
To download the **pokeyellow-fr** source files:
...
If you have different projects that require different versions of `rgbds`, it might not be convenient to install rgbds 1.0.3 globally. Instead, you can put its files in a directory within pokeyellow-fr, such as `pokeyellow-fr/rgbds-1.0.3/`. Then specify it when you run `make`:
